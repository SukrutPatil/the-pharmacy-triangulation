...
export class AuthController {
  ...
  @Post('login')
  async postLoginInformation(
    @Req() req: Request,
    @Res() res: Response,
  ): Promise<any> {
    const { email, password } = req.body;
    const theResultSet = await this.DBService.retrieve(
      EntryType.MEMBER,
      `where email = '${email}'`,
    );
    if (theResultSet.status == QueryStatus.FAILED) {
      ...
      // If there are more than one row
        ...
        console.log(
          `Multiple Rows Captured At Database on entering email = ${email} and password = ${password}`,
        );
